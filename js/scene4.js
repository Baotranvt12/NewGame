...
var check4_1;
var check4_2;
var list4 = ["bong thom no canh bia rung", "ong ve chua dam dau, lu buom dung lau chau"];
var arrow;
...
class scene4 extends Phaser.Scene{
	...
	preload(){
		...
		this.load.image('s4_1', 'assets/s4_1.png');
		this.load.image('s4_2', 'assets/s4_2.png');
		this.load.image('check4_1', 'assets/check4_1.png');
		this.load.image('check4_2', 'assets/check4_2.png');
	...
	create(){
		this.add.image(610, 380, 'bgs1'); //background
		...
		//hai câu tục ngữ
		s4_1 = this.add.sprite(600, 300, "s4_1").setInteractive();
		s4_2 = this.add.sprite(600, 350, "s4_2").setInteractive();
		
		//mũi tên qua màn
		arrow = this.add.sprite(595, 540, "arrow").setInteractive();
		arrow.visible = false;

		check4_1 = this.add.sprite(600,300, 'check4_1').setInteractive();
		check4_2 = this.add.sprite(600,350, 'check4_2').setInteractive();
		
		//ẩn đáp án
		check4_1.visible = false;
		check4_2.visible = false;
		
		///xử lý chữ nhập vào và con ong trên ngón tay
		//begin
		this.typedMessage = "";
		this.typedGameObject = this.add.text(200, 645,  "Hello world", {
			fontSize:32, color:'#c51b7d'
		...
		if(this.typedMessage != ''){
			...
			checkButton.on("pointerdown", () => {
				if (this.typedMessage === validText) {
					...
					//hiện đáp án đúng
					if(index == 0)
						check4_1.visible = true;
					else{
						check4_2.visible = true;
						arrow.visible = true;
					}
